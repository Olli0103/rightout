...
function validateStoredPlan(plan) {
    ...
    if (plan.receipt !== null) {
        if (!plan.receipt || typeof plan.receipt !== "object" || Array.isArray(plan.receipt)
...
export function workerPolicyDigest({ catalogDigest, recipeDigest, runtimeScopeDigest, marketPolicyDigest }) {
    if (![catalogDigest, recipeDigest, runtimeScopeDigest, marketPolicyDigest].every((value) => typeof value === "string" && SAFE_SHA256.test(value))) {
        throw new Error("rightout_worker_policy_invalid");
    }
    return createHash("sha256").update(JSON.stringify([
        "rightout-worker-policy-v2", catalogDigest, recipeDigest, runtimeScopeDigest, marketPolicyDigest,
    ])).digest("hex");
}
...
export function classifyWorkerExecutionResult(toolName, result, error) {
    ...
    if (!SAFE_RESULT_STATE.test(resultState))
...
export function createAutonomyWorkerLedger(store, { now = () => Date.now(), randomWorkerId = (campaignId) => `worker_${createHash("sha256").update(JSON.stringify(["rightout-worker-v1", campaignId])).digest("hex").slice(0, 32)}`, randomLeaseId = () => `lease_${randomBytes(16).toString("hex")}`, } = {}) {
    ...
    async function matchExecution(tool, parameters, sessionBindingDigest) {
        ...
        for (const entry of await store.entries()) {
            ...
            let clean;
            try {
                clean = validateParameters(parameters, tool, record);
            }
            catch {
                continue;
            }

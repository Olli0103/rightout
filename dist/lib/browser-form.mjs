...
function challengeClass(text) {
    ...
    if (/\bcaptcha\b/u.test(clean) && !/\barithmetic captcha\b/u.test(clean))
        return "hard_human_gate";
...
function parsedArithmeticChallenge(text) {
    const matches = [...String(text ?? "").matchAll(/(?:arithmetic (?:challenge|captcha)|static arithmetic challenge)[^0-9-]{0,80}(-?\d{1,4})\s*([+*x×−-])\s*(-?\d{1,4})(?:\s*=\s*\?)?/giu)];
    if (matches.length !== 1)
        return null;
    ...
    if (!Number.isSafeInteger(answer) || Math.abs(answer) > 100_000_000)
        return null;
    ...
    if (privacyMode === "generic_form") {
        ...
        refs = normalized.flatMap((item) => {
            ...
            if (["textbox", "combobox"].includes(item.role)) {
                ...
                if (matches.length === 1)
                    return [{ ref: item.ref, role: item.role, name: matches[0][2] }];
                return [];
            }
            if (!["button", "link", "checkbox", "radio"].includes(item.role))
                return [];
            ...
            if (corroboratedRecord)
                return [{ ref: item.ref, role: item.role, name: "corroborated subject record", corroborated: true }];
        ...
        refs = normalized.flatMap((item) => {
            ...
            if (nameField)
                return [{ ref: item.ref, role: item.role, name: "legal name field" }];
            if (dobField)
                return [{ ref: item.ref, role: item.role, name: "date of birth field" }];
            if (continueAction)
                return [{ ref: item.ref, role: item.role, name: "continue action" }];
            if (corroboratedRecord)
                return [{ ref: item.ref, role: item.role, name: "corroborated subject record", corroborated: true }];
            if (suppressAction)
                return [{ ref: item.ref, role: item.role, name: "suppression action" }];
            return [];
        });
        text = [
...
function exactRef(snapshot, ref, roles) {
    if (typeof ref !== "string" || !SAFE_REF.test(ref))
        throw new Error("rightout_form_ref_invalid");
...
function assertFieldPurpose(item, profileField, allowedFields) {
    ...
    if (!FIELD_PURPOSE_PATTERNS[profileField]?.test(semanticName))
        throw new Error("rightout_form_field_target_mismatch");
    const matches = allowedFields.filter((field) => FIELD_PURPOSE_PATTERNS[field]?.test(semanticName));
    if (matches.length !== 1 || matches[0] !== profileField)
        throw new Error("rightout_form_field_target_ambiguous");
}
        ...
        : profileField === "phone" ? "tel"
            : profileField === "listing_url" ? "url"
                : profileField === "date_of_birth" ? "date"
                    : "text";
    if (type !== expected)
        throw new Error("rightout_form_field_type_mismatch");
}
...
export function createBrowserSessionDriver({ fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
    ...
    async function rawSnapshot(options) {
        ...
        return bridgeRequest(fetchImpl, base, `/snapshot?format=ai&refs=aria&interactive=true&compact=true&targetId=${encodeURIComponent(options.targetId)}&maxChars=100000&timeoutMs=20000`, { signal: options.signal, profile: options.browserProfile, authToken: options.browserAuthToken });
    }
    ...
    async function captureCandidate(options) {
        ...
        candidate.hash = "";
        start.hash = "";
        if (candidate.toString() === start.toString())
            throw new Error("rightout_discovery_candidate_not_selected");
        return {
    ...
    async function act(options) {
        ...
        try {
            if (action.kind === "fill") {
                if (!Array.isArray(action.fields) || action.fields.length < 1 || action.fields.length > 12)
                    throw new Error("rightout_form_action_not_allowed");
                ...
                const fields = action.fields.map((field) => {
                    ...
                    if (!new Set(["text", "email", "tel", "url", "date"]).has(field.type))
                        throw new Error("rightout_form_action_not_allowed");
                ...
                if (before.challenge !== "static_challenge_visible" || item.name !== "arithmetic answer")
                    throw new Error("rightout_form_action_not_allowed");
                const arithmetic = before.arithmetic_challenge;
                if (!arithmetic || !Number.isInteger(arithmetic.left) || !Number.isInteger(arithmetic.right))
                    throw new Error("rightout_form_action_not_allowed");
                ...
                if (options.privacyMode === "peopleconnect_guided" && action.purpose === "select_record"
                ...
                if (["generic_form", "publisher_discovery"].includes(options.privacyMode ?? "generic_form") && action.purpose === "select_record") {
                    if (item.corroborated !== true)
                        throw new Error("rightout_form_record_not_corroborated");
            ...
            if (externalEffectStarted)
                throw new Error("rightout_browser_action_uncertain");
            throw error;
        }
    }
    ...
    async function discardDraft(options) {
        ...
        if (matches.length !== 1)
            return { discarded: false, reason: "discard_control_not_observed" };
        await bridgeRequest(fetchImpl, base, "/act", {
            method: "POST",
...
export function createBrowserFormSubmitter({ fetchImpl = globalThis.fetch, now = () => new Date() } = {}) {
    ...
    return async function submitBrowserForm({ bridgeUrl, formUrl, recipe, values, signal, browserProfile, browserAuthToken }) {
        ...
        try {
            ...
            if (submitStarted)
                throw new Error("rightout_form_submission_uncertain");
            throw error;
        }
        finally {
            if (targetId) {
                try {
                    await bridgeRequest(fetchImpl, base, `/tabs/${encodeURIComponent(targetId)}`, { method: "DELETE", signal: undefined, profile: browserProfile, authToken: browserAuthToken });
                }

...
export function validateCampaignStartInput(value) {
    ...
    if (Object.keys(value).some((key) => !allowed.has(key)))
        throw new Error("rightout_campaign_scope_invalid");
    if (typeof value.profileId !== "string" || !SAFE_PROFILE_ID.test(value.profileId))
        throw new Error("rightout_campaign_scope_invalid");
    ...
    if (effects.some((effect) => !EFFECT_SET.has(effect)))
        throw new Error("rightout_campaign_scope_invalid");
...
function validateRequestedEffects(value) {
    ...
    return value.map((item) => {
        if (!item || typeof item !== "object" || Array.isArray(item))
            throw new Error("rightout_campaign_effect_invalid");
        if (Object.keys(item).some((key) => !["brokerId", "effect"].includes(key)))
            throw new Error("rightout_campaign_effect_invalid");
...
export function campaignApprovalDescription(input, routingScope = {
    ...
    if (!["openclaw_sandbox_browser_bridge", "standalone_loopback_http_opt_in", "not_required"].includes(browserControlTransport)
...
export function createCampaignLedger(store, { now = () => Date.now(), randomId = () => `campaign_${randomBytes(16).toString("hex")}`, } = {}) {
    ...
    async function assertScope(campaignId, { profileId, profileDigest, runtimeScopeDigest }) {
        ...
        if (record.profileDigest !== profileDigest)
    ...
    async function consume(campaignId, { profileId, effects, catalogDigest, profileDigest, runtimeScopeDigest }) {
        ...
        await store.update(campaignId, (current) => {
            ...
            if (record.profileId !== profileId || record.catalogDigest !== catalogDigest)
                throw new Error("rightout_campaign_scope_mismatch");
            if (record.profileDigest !== profileDigest)
                throw new Error("rightout_profile_snapshot_changed");
            if (record.runtimeScopeDigest !== runtimeScopeDigest)
            ...
            if (record.usedEffects + requested.length > record.maxEffects)
                throw new Error("rightout_campaign_effect_budget_exhausted");

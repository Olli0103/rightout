...
export function createCaseLedger(store, { now = () => new Date() } = {}) {
    ...
    async function recordControllerOutcome(profileId, brokerId, outcome, broker) {
        ...
        if (!broker || broker.id !== cleanBroker || !["eu_controller_email_erasure", "uk_controller_email_erasure", "us_data_broker_email_deletion"].includes(broker.process_class)
        ...
        if ((broker.process_class === "eu_controller_email_erasure" && processingDays !== 30)
            ...
            throw new Error("unsupported_controller_outcome_lane");
        if ((["eu_controller_email_erasure", "uk_controller_email_erasure"].includes(broker.process_class) && ["deletion_confirmed", "partial_deletion"].includes(outcome))
    ...
    async function recordDropStatus(profileId, observedStatus) {
        ...
        return withProfile(cleanProfile, (profile, at) => {
            ...
            if (!brokerCase || brokerCase.mechanism !== "california_drop"
            ...
            if (expectedSchedule.contract_digest !== brokerCase.mechanism_contract_digest
            ...
            if (observedStatus === "deleted" && !processingStarted)
                throw new Error("rightout_drop_status_invalid");

...
function safeOfficialActionUrl(broker) {
    const raw = broker?.eu_process?.official_action_url ?? broker?.official_url;
...
function nextActionFor(broker, brokerCase) {
    if (brokerCase.state === "confirmed_removed")
        return "wait_for_reappearance_recheck";
    if (["submitted", "verification_pending"].includes(brokerCase.state)) {
        return broker.removal?.confirmation_policy === "submitted_until_controller_response"
            ? "wait_for_controller_response"
            : "poll_verification";
    }
    if (brokerCase.state === "awaiting_processing")
        return "wait_for_due_direct_rescan";
...
export function createCaseLedger(store, { now = () => new Date() } = {}) {
    ...
    async function recordRemoval(report, processingDays = DEFAULT_PROCESSING_DAYS) {
        ...
        return withProfile(profileId, (profile) => {
            ...
            if (brokerCase.state === "new"
                && report.discovery_requirement === "not_required_for_data_subject_request") {
    ...
    async function recordDirectRescan(report, rescanDays = DEFAULT_RESCAN_DAYS) {
        ...
        return withProfile(profileId, (profile) => {
            ...
            if (report.observation === "direct_present") {
                ...
                if (brokerCase.state === "awaiting_processing")
                    transition(brokerCase, "confirmed_removed", at, "trusted_direct_rescan_absent");
                else if (brokerCase.state !== "confirmed_removed")
                    transition(brokerCase, "not_found", at, "trusted_direct_rescan_absent_known_listing_set");
                if (brokerCase.state === "confirmed_removed") {
    ...
    async function plan(profileId, catalog) {
        ...
        const rows = catalogRows(catalog).map((broker) => {
            ...
            const officialActionUrl = broker.eu_process && typeof broker.eu_process === "object"
                ? safeOfficialActionUrl(broker)
                : undefined;
            return {
                broker_id: broker.id,
                state: brokerCase.state,
        ...
        const order = { refresh_catalog: 0, run_discovery: 1, poll_verification: 2, wait_for_due_direct_rescan: 3, submit_email_removal: 4, submit_browser_form: 5, queue_human_task: 6, complete_human_task: 7, retry_or_route_human: 8, wait_for_cluster_parent: 9, wait_for_controller_response: 10, wait_for_reappearance_recheck: 11 };
        rows.sort((a, b) => {

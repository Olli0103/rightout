...
function assertSanitized(model) {
    ...
    try {
        ...
        throw new Error("rightout_dashboard_invalid");
    }
...
async function privateDirectory(root) {
    ...
    if (!rel || rel.startsWith("..") || isAbsolute(rel))
        throw new Error("rightout_dashboard_path_invalid");
    try {
        await mkdir(directory, { mode: 0o700 });
    }
    catch (error) {
        if (error?.code !== "EEXIST")
            throw new Error("rightout_dashboard_export_failed");
    }
...
export async function exportLocalDashboard(modelInput, root, format = "html") {
    if (!new Set(["html", "json"]).has(format))
        throw new Error("rightout_dashboard_format_invalid");

...
function cleanInput(input) {
    ...
    if (!SAFE_HANDLE.test(input?.listingHandle))
...
function subjectSignals(profilePayload) {
    let value;
    try {
        value = JSON.parse(profilePayload);
    }
    catch {
    ...
    for (const item of locations)
        if (normalizedText(item).length >= 3)
            corroborators.push({ kind: "location", value: normalizedText(item) });
    for (const item of [value.contactEmail, ...(Array.isArray(value.emails) ? value.emails : [])]) {
        if (normalizedText(item).includes("@"))
            corroborators.push({ kind: "email", value: normalizedText(item) });
    }
    for (const item of [value.address, ...(Array.isArray(value.priorAddresses) ? value.priorAddresses : [])]) {
        if (normalizedText(item).length >= 6)
            corroborators.push({ kind: "address", value: normalizedText(item) });
    }
    for (const item of Array.isArray(value.phones) ? value.phones : []) {
        const digits = String(item).replace(/\D/g, "");
        if (digits.length >= 7)
            corroborators.push({ kind: "phone", value: digits });
    }
    if (!corroborators.length)
        throw new Error("direct_rescan_corroborator_required");
...
async function boundedText(response, signal) {
    ...
    try {
        while (true) {
            if (signal?.aborted)
                throw new Error("rightout_direct_scan_cancelled");
...
function pageMatch(html, signals) {
    ...
    for (const item of signals.corroborators) {
        ...
        if (haystack.includes(item.value))
            return { matched: true, corroborator: item.kind };
    }
    return { matched: false };
}
export async function runDirectRescan({ input, catalog, profilePayload, attestations, token, guardedFetch, signal }) {
    ...
    for (const url of token.urls) {
        ...
        try {
            ...
            if ([404, 410].includes(request.response.status))
                ...
                if (match.blocked)
                    ...
                    matchedCorroborator = match.corroborator;
                }
                else
                    observations.push("ambiguous");
            }
            else
                observations.push("ambiguous");
        }
        catch (error) {
            if (signal?.aborted || (error instanceof Error && error.message === "rightout_direct_scan_cancelled"))
                throw new Error("rightout_direct_scan_cancelled");

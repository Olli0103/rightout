...
function durationSummary(values) {
    if (!values.length)
        return { evidenced: 0, minimum_hours: null, median_hours: null, average_hours: null, maximum_hours: null };
...
export function buildEffectivenessReport(caseStatus, canaryFacts) {
    if (!caseStatus || typeof caseStatus !== "object" || !SAFE_PROFILE_ID.test(caseStatus.subject_ref ?? "")
    ...
    if (canaries.some((item) => (Date.parse(item.observedAt) > Date.parse(caseStatus.generated_at)

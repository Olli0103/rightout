...
function cleanInput(input) {
    ...
    if (input.requestKind !== "delete_and_opt_out")
...
function cleanSpec(value, expectedRoles) {
    ...
    if (expectedRoles && roles.some((role) => !expectedRoles.has(role)))
        throw new Error("unsupported_form_lane");
...
function cleanRecipe(value) {
    ...
    const fields = value.fields.map((field) => cleanSpec(field, new Set(["textbox"]))).map((field) => {
        if (field.profile_field !== "contact_email" || field.type !== "text")
            throw new Error("unsupported_form_lane");
        return field;
    });
    if (!Array.isArray(value.checkboxes) || value.checkboxes.length !== 1)
        throw new Error("unsupported_form_lane");
...
export function resolveFormCatalogEntry(catalog, input) {
    ...
    if (!broker || broker.category !== "people_search" || broker.lane !== "browser_form"
    ...
    let formUrl;
    try {
        formUrl = new URL(removal.form_url);
    }
    catch {
        throw new Error("unsupported_form_lane");
    }
    ...
    if (JSON.stringify(removal.disclosure_fields) !== JSON.stringify(["contact_email"]))
        throw new Error("unsupported_form_lane");
    return {
        id: broker.id,
...
export function validateFormAttestations(input, value) {
    ...
    if (!profileIds.includes(publicInput.profileId) || !brokerIds.includes(publicInput.brokerId))
        throw new Error("rightout_form_attestation_required");
    return {

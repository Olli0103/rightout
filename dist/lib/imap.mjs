...
function hasAlignedDkimPass(headers, allowedSenderDomains, imapHost) {
    ...
    if (!trustedAuthservId || values.length !== 1)
        return false;
    const authenticationResults = values[0].toLowerCase();
    const separator = authenticationResults.indexOf(";");
    if (separator < 1 || authenticationResults.slice(0, separator).trim() !== trustedAuthservId)
        return false;
    if (!authenticationResults || /\bdkim=(?:fail|permerror|temperror|neutral|none)\b/u.test(authenticationResults))
        return false;
...
export function extractBoundVerificationLink({ text, html, senderDomains, allowedSenderDomains, allowedLinkDomains }) {
    ...
    if (!senders.some((domain) => hostMatches(domain, allowedSenderDomains)))
        return undefined;
...
function verificationLane(broker) {
    ...
    if (verification?.supported !== true || verification.channel !== "imap")
        throw new Error("unsupported_verification_lane");
    return {
...
export function createImapPoller({ clientFactory = (options) => new ImapFlow(options), parser = simpleParser, now = () => new Date(), } = {}) {
    return async function pollBrokerVerification({ transport, expectedAddress, broker, notBefore, sinceDays = 14, signal }) {
        if (!Number.isInteger(sinceDays) || sinceDays < 1 || sinceDays > 30)
            throw new Error("invalid_verification_window");
        if (signal?.aborted)
        ...
        if (typeof notBefore !== "string" || !Number.isFinite(submittedAt.getTime()))
            throw new Error("invalid_verification_submission_time");
        ...
        try {
            ...
            for (const uid of selected) {
                ...
                if (!(messageDate instanceof Date) || !Number.isFinite(messageDate.getTime()) || messageDate.getTime() < submittedAt.getTime())
                    continue;
                if (!addressValues(parsed.to?.value).includes(config.address))
                    continue;
                if (!hasAlignedDkimPass(parsed.headers, lane.senderDomains, config.host))
                    continue;
            ...
            if (error instanceof Error && error.message === "rightout_verification_cancelled")
                throw error;
            if (signal?.aborted)
                throw new Error("rightout_verification_cancelled");

...
function cleanScanConsent(value) {
    ...
    if (scope.length !== value.scope.length
...
function boundedBraveQuery(officialDomain, requiredTerm, optionalTerms = []) {
    ...
    if (!braveQueryWithinLimits(query))
        return null;
    for (const term of optionalTerms) {
        const candidate = `${query} "${term}"`;
        if (braveQueryWithinLimits(candidate))
            query = candidate;
    }
    return query;
}
export function buildSearchVectors(subject, officialDomain) {
    ...
    for (const phone of subject.phones ?? [])
        vectors.push({ kind: "phone", query: boundedBraveQuery(officialDomain, phone) });
...
export async function runLiveScan({ input, catalog, apiKey, guardedFetch, signal, operatorAttestations, approvalBoundary = "assisted_allow_once", }) {
    ...
    const results = await mapBounded(selected, 4, async (broker) => {
        ...
        for (const vector of vectors) {
            try {
                const payload = await guardedJsonPost(guardedFetch, BRAVE_ENDPOINT, { q: vector.query, country: braveLocale.country, search_lang: braveLocale.search_lang, safesearch: "strict", count: 10 }, apiKey, signal);
                attempted += 1;

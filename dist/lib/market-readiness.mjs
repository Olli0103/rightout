...
function californiaDropPhase(now) {
    ...
    if (now < launch)
        return "not_launched";
    if (now < processing)
        return "consumer_requests_open_broker_processing_not_started";
    return "broker_processing_required";
}
function validatePolicy(policy) {
    if (!policy || typeof policy !== "object" || !SAFE_MARKET_ID.test(policy.market_id)
...
export function marketPolicyHealth({ now = Date.now() } = {}) {
    ...
    const summary = {
        ...
        stale: count("stale"),
        rights_execution_core_markets: markets.filter((market) => (["catalog_limited_18_email_routes", "catalog_limited_1_uk_email_route", "catalog_limited_email_routes"]
            .includes(market.rightout_support.controller_request))).length,
        human_or_unsupported_markets: markets.filter((market) => (!["catalog_limited_18_email_routes", "catalog_limited_1_uk_email_route", "catalog_limited_email_routes"]
            .includes(market.rightout_support.controller_request))).length,
    };
    if (Object.keys(summary).some((key) => key.includes("pii")))
        throw new Error("rightout_market_policy_invalid");
    return {
        report_version: 1,
...
export function marketIdsForJurisdictions(jurisdictions) {
    if (!Array.isArray(jurisdictions) || jurisdictions.length < 1 || jurisdictions.length > 20
    ...
    for (const jurisdiction of jurisdictions) {
        if (["EU", "EEA"].includes(jurisdiction))
            ids.add("eu_eea");
        else if (["UK", "GB"].includes(jurisdiction))
            ids.add("uk");
        else if (jurisdiction === "US-CA")
            ids.add("us_california");
...
export function assertMarketRightsExecution({ jurisdictions = [], executionClass = "controller_request", now = Date.now() } = {}) {
    ...
    if (!market)
        throw new Error("rightout_market_execution_unsupported");
    if (market.source_status !== "current")
        throw new Error("rightout_market_policy_source_not_current");
    if (executionClass === "controller_request"
 ...
 */
export function assertMarketRouteExecution({ jurisdictions = [], marketIds = [], executionClass = "provider_delete_opt_out", now = Date.now(), } = {}) {
    ...
    if (!Array.isArray(marketIds) || marketIds.length < 1 || marketIds.length > 11
...
export function assertProfileEligibleForMarketRoute({ routeJurisdictions = [], profileJurisdictions = [] } = {}) {
    ...
    if (!matched.length)
        throw new Error("rightout_market_profile_ineligible");

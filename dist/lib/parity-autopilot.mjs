...
function assertCaseStatus(value) {
    if (!value || typeof value !== "object" || !Array.isArray(value.cases))
        throw new Error("rightout_autopilot_state_invalid");
}
 ...
 */
export function planParityCampaignNext({ campaign, caseStatus, parityCatalog, coreCatalog, emailMode = "smtp", verificationMode = "imap", browserMode = "managed_openclaw", remoteCloudRetryAvailable = false, now = Date.now(), }) {
    assertPublicCampaign(campaign);
    assertCaseStatus(caseStatus);
    if (!parityCatalog || !Array.isArray(parityCatalog.brokers))
        throw new Error("rightout_autopilot_catalog_invalid");
    if (!["smtp", "webmail", "unavailable"].includes(emailMode))
        throw new Error("rightout_autopilot_transport_invalid");
    if (!["imap", "unavailable"].includes(verificationMode))
        throw new Error("rightout_autopilot_transport_invalid");
    ...
    if (scopedRoutes.length !== campaign.broker_ids.length)
        throw new Error("rightout_autopilot_catalog_invalid");
    ...
    for (const { route, item } of actionable) {
        ...
        if (cluster?.role === "child"
            ...
            if (["found", "indirect_exposure", "action_selected", "reappeared", "submission_pending", "submission_uncertain", "submitted", "verification_pending", "awaiting_processing", "confirmed_removed"].includes(parentState))
                continue;
        }
    ...
    for (const { route, item } of due) {
        ...
        if (route.verification === "email" && effects.has("poll_verification")) {
            ...
            deferredHumanGates.push({
                ...
                next_action: "configure_receiver_authenticated_imap_or_complete_verification_manually",
            });
        }

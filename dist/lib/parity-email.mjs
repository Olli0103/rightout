...
function cleanBroker(value, input) {
    if (!value || value.id !== input.brokerId || typeof value.name !== "string" || !SAFE_EMAIL.test(value.rescue_email)
...
export function buildParityMessage({ input, broker, profilePayload, listingUrl }) {
    ...
    if (route.disclosureFields.includes("listing_url") && typeof listingUrl !== "string")

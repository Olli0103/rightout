...
function safePath(root, child) {
    ...
    if (!rel || rel.startsWith("..") || isAbsolute(rel))
        throw new Error("rightout_transaction_lock_path_invalid");
    return target;
}
async function privateStateDirectory(stateDir) {
    ...
    try {
        ...
        if (error?.code !== "EEXIST")
            throw new Error("rightout_transaction_lock_directory_unsafe");
    }
    ...
    if (canonical !== directory || dirname(canonical) !== trustedRoot)
        throw new Error("rightout_transaction_lock_directory_unsafe");
    return directory;
}
            ...
            try {
                ...
                try {
                    ...
                    if (metadataError?.code !== "ENOENT")
                        throw new Error("rightout_transaction_lock_unsafe");
                    await new Promise((resolveWait) => setTimeout(resolveWait, 25));
                    continue;
                }
                if (!metadata.isDirectory() || metadata.isSymbolicLink())

...
OpenClaw blocks on denial, timeout, cancellation, missing operator attestations, hook failure, or missing approval route. Before offering approval, RightOut requires operator-owned subject authorization, Brave terms revision `2026-02-11`, Brave customer-responsibility acceptance, and exact broker search scope. On `allow-once`, RightOut stores only a short-lived single-use binding between the host tool-call ID and the complete normalized scope shown in the prompt. Execution consumes it and rechecks attestations before accessing already materialized config values or making the one Brave request; replay, expiry, direct invocation, or parameter mutation fails closed. The approval also states index-only/no-publisher-fetch behavior and Brave's published standard-plan query-log maximum of 90 days unless an applicable ZDR agreement exists.

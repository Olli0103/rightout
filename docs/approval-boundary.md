...
`/tools/invoke` is a full-operator surface. Production guidance denies all nine approval-gated tools through `gateway.tools.deny` unless direct operator invocation is intended. Read-only case tools can remain available.

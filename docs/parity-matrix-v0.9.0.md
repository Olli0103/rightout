...
| Durable worker | encrypted state, live atomic leases, exact result receipts, checkpoints, backoff, session/policy binding, cross-process state-token schedule replacement, resume/revoke tests | passed |
...
| Evidence vault | encryption, content addressing, retention, tamper, cross-instance/process serialized export/purge/rotation, approved export | passed |

...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const workerEffectCalls = new Map<string, {
      workerId: string;
      leaseId: string;
    ...
    function workerHookRunId(event: { runId?: string }, context: Record<string, any>): string {
      const eventRunId = event.runId;
      const contextRunId = context?.runId;
      if (
        typeof eventRunId !== "string" || !/^[A-Za-z0-9._:-]{8,160}$/.test(eventRunId)
        || typeof contextRunId !== "string" || contextRunId !== eventRunId
      ) throw new Error("rightout_worker_run_required");
      return eventRunId;
    }

    function workerEffectCallKey(sessionBindingDigest: string, runId: string, toolCallId: string): string {
    ...
    async function recoverWorkerSchedules(): Promise<void> {
      for (const worker of await workerLedger.recoverable()) {
        ...
        try {
          const scheduled = await scheduleWorkerRoute(
            ...
            worker.schedule_token,
          );
          if (scheduled.scheduler_state === "stale_recovery_skipped") continue;
          if (scheduled.scheduled_job_registered !== true) {
            if (await workerLedger.scheduleToken(worker.worker_id) !== worker.schedule_token) continue;
    ...
    api.on("before_tool_call", async (event, hookContext) => {
      ...
      if (event.toolCallId && event.toolName.startsWith("rightout_") && typeof hookContext?.sessionKey === "string" && typeof hookContext?.agentId === "string") {
        try {
          ...
          if (matched) {
            const runId = workerHookRunId(event, hookContext as Record<string, any>);
            const key = workerEffectCallKey(session.sessionBindingDigest, runId, event.toolCallId);
            if (workerEffectCalls.has(key)) throw new Error("rightout_worker_call_ref_duplicate");
    ...
    api.on("after_tool_call", async (event, hookContext) => {
      ...
      let key;
      try {
        session = trustedWorkerSession(hookContext as Record<string, any>);
        runId = workerHookRunId(event, hookContext as Record<string, any>);
        key = workerEffectCallKey(session.sessionBindingDigest, runId, event.toolCallId);
      } catch { return; }
      const pending = workerEffectCalls.get(key);
      if (!pending || pending.toolName !== event.toolName || pending.runId !== runId || pending.sessionBindingDigest !== session.sessionBindingDigest) return;
      let matched;
      try { matched = await workerLedger.matchExecution(event.toolName, event.params, session.sessionBindingDigest); }
      catch { return; }
      if (
        !matched || matched.worker_id !== pending.workerId || matched.lease_id !== pending.leaseId
      ...
      (toolContext) => ({
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            try { await workerLedger.gateRecovery(input.workerId, "post_claim_tick_failed"); }
            catch { api.logger?.warn?.("RightOut failed to persist the post-claim worker gate; operator recovery is required"); }
            throw error;
          }
        },
      }),

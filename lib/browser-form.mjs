...
function challengeClass(text) {
  ...
  if (/\bstatic text challenge\b/u.test(clean)) return "hard_human_gate";
  return "none";
}
...
function publicSessionSnapshot(snapshot, {
  allowedDomains, allowedFields = [], values, privacyMode = "generic_form", brokerMessageDomains = [], verificationRecipient,
  verificationMessageAuthenticated = false,
}) {

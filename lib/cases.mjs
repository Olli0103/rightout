...
export function createCaseLedger(store, { now = () => new Date() } = {}) {
  ...
  async function recordControllerOutcome(profileId, brokerId, outcome, broker) {
    ...
    if (
      !broker || broker.id !== cleanBroker || !["eu_controller_email_erasure", "us_data_broker_email_deletion"].includes(broker.process_class)
    ...
    if (
      (broker.process_class === "eu_controller_email_erasure" && ["deletion_confirmed", "partial_deletion"].includes(outcome))
  ...
  async function recordDropFiled(profileId, { registryCount, processingStart }) {
    ...
    return withProfile(cleanProfile, (profile, at) => {
      ...
      if (existing && ["submitted", "awaiting_processing", "confirmed_removed"].includes(existing.state)) throw new Error("rightout_drop_already_recorded");

...
import { calculateDropSchedule, DROP_CONTRACT, dropContractDigest } from "./drop.mjs";
...
function safeOfficialActionUrl(broker) {
  const raw = broker?.eu_process?.official_action_url ?? broker?.uk_process?.official_action_url
    ?? broker?.us_process?.official_action_url ?? broker?.human_action_url ?? broker?.official_url;
...
function nextActionFor(broker, brokerCase, at = new Date().toISOString()) {
  ...
  const directAction = () => {
    if (broker.direct_rescan?.supported !== true) return "review_provider_status_human";
    return SAFE_LISTING_HANDLE.test(brokerCase.listing_handle ?? "") ? "run_direct_rescan" : "refresh_discovery_for_direct_handle";
  };
  if (brokerCase.state === "confirmed_removed") return isDue ? directAction() : "wait_for_reappearance_recheck";
  if (["submission_pending", "submission_uncertain"].includes(brokerCase.state)) return "reconcile_submission";
  if (brokerCase.state === "identity_verification_required") return "complete_identity_verification_human_task";
  if (brokerCase.state === "partially_removed") return "review_partial_controller_outcome";
  if (brokerCase.state === "request_rejected") return "review_rejection_or_escalate";
  ...
  if (brokerCase.state === "awaiting_processing") return isDue ? directAction() : "wait_for_due_direct_rescan";
...
export function createCaseLedger(store, { now = () => new Date() } = {}) {
    ...
    return withProfile(profileId, (profile) => {
      for (const result of report.results) {
        ...
        if (result.listing_handle !== undefined) {
          if (typeof result.listing_handle !== "string" || !SAFE_LISTING_HANDLE.test(result.listing_handle)) throw new Error("invalid_scan_report");
  ...
  async function recordBrowserDiscovery(report) {
    if (
      !report || report.mode !== "operator_authorized_browser_discovery"
  ...
  async function recordRemoval(report, processingDays = DEFAULT_PROCESSING_DAYS) {
    ...
    const explicitRecheck = report.response_window?.conservative_recheck_at;
    const nextRecheckAt = explicitRecheck === undefined
      ? addDays(at, processingDays)
      : safeDate(explicitRecheck, "removal_recheck_timestamp");
    if (
      explicitRecheck !== undefined
      && (
        report.process_class !== "uk_controller_email_erasure"
        || report.response_window?.policy !== "one_calendar_month_conservative_recheck_v1"
        || Date.parse(nextRecheckAt) <= Date.parse(at)
        || Date.parse(nextRecheckAt) - Date.parse(at) > 32 * 24 * 60 * 60_000
      )
    ) throw new Error("invalid_processing_window");
    return withProfile(profileId, (profile) => {
      const brokerCase = profile.brokers[brokerId] ?? newBrokerCase(brokerId, at);
      if (brokerCase.state !== "submission_pending") throw new Error("submission_intent_required");
      ...
      brokerCase.next_recheck_at = nextRecheckAt;
      profile.brokers[brokerId] = brokerCase;
    });
  }
  ...
  async function recordSuppressionSubmission(report, processingDays = DEFAULT_PROCESSING_DAYS) {
    if (
      !report || report.state !== "awaiting_processing" || report.delivery?.form_submitted !== true
    ...
    return withProfile(profileId, (profile) => {
      ...
      if (report.observation === "direct_present") {
        ...
        if (removalInFlight && !priorAbsence) {
          if (stateBeforeAbsence !== "awaiting_processing") transition(brokerCase, "awaiting_processing", at, "first_trusted_direct_rescan_absent");
          else transition(brokerCase, "awaiting_processing", at, "first_trusted_direct_rescan_absent_after_processing_start");
          brokerCase.direct_absence_observed_at = at;
          if (!brokerCase.next_recheck_at || brokerCase.next_recheck_at <= at) brokerCase.next_recheck_at = addDays(at, 7);
  ...
  async function deferRecheck(profileId, brokerId, { reason, days = 1 } = {}) {
    ...
    if (!Number.isInteger(days) || days < 1 || days > 30) throw new Error("invalid_recheck_window");
    return withProfile(cleanProfile, (profile, at) => {
      const brokerCase = profile.brokers[cleanBroker];
      if (!brokerCase) throw new Error("case_not_found");
  ...
  async function reserveSubmission(profileId, brokerId, { channel, discoveryRequirement } = {}) {
    const cleanProfile = safeProfileId(profileId);
    const cleanBroker = safeBrokerId(brokerId);
    if (!["smtp_email", "browser_webmail", "browser_form"].includes(channel)) throw new Error("invalid_submission_channel");
  ...
  async function releaseSubmission(profileId, brokerId, reason = "provider_write_not_started") {
    ...
    return withProfile(cleanProfile, (profile, at) => {
      const brokerCase = profile.brokers[cleanBroker];
      if (!brokerCase || brokerCase.state !== "submission_pending") throw new Error("rightout_submission_not_pending");
  ...
  async function recordControllerOutcome(profileId, brokerId, outcome, broker) {
    ...
    if (!allowedOutcomes.has(outcome)) throw new Error("invalid_controller_outcome");
    if (
      !broker || broker.id !== cleanBroker || !["eu_controller_email_erasure", "uk_controller_email_erasure", "us_data_broker_email_deletion"].includes(broker.process_class)
    ...
    if (
      (broker.process_class === "eu_controller_email_erasure" && processingDays !== 30)
      || (broker.process_class === "uk_controller_email_erasure" && processingDays !== 28)
      || (broker.process_class === "us_data_broker_email_deletion" && processingDays !== 45)
    ) throw new Error("unsupported_controller_outcome_lane");
    if (
      (["eu_controller_email_erasure", "uk_controller_email_erasure"].includes(broker.process_class) && ["deletion_confirmed", "partial_deletion"].includes(outcome))
    ...
    return withProfile(cleanProfile, (profile, at) => {
      ...
      brokerCase.human_task_reason = outcome === "identity_required"
        ...
        : ["partial_erasure", "partial_deletion", "request_rejected"].includes(outcome)
          ...
          : null;
      if (target === "awaiting_processing") brokerCase.next_recheck_at = addDays(at, processingDays);
  ...
  async function recordLifecycle(profileId, brokerId, state, options = {}) {
    ...
    if (state === "confirmed_removed" || state === "reappeared") throw new Error("trusted_rescan_or_controller_method_required");
    return withProfile(cleanProfile, (profile, at) => {
      const brokerCase = profile.brokers[cleanBroker] ?? newBrokerCase(cleanBroker, at);
  ...
  async function recordDropFiled(profileId, { registryCount, registrySnapshotDigest }) {
    const cleanProfile = safeProfileId(profileId);
    if (!Number.isInteger(registryCount) || registryCount < 1 || registryCount > 10_000) throw new Error("rightout_drop_registry_invalid");
    if (typeof registrySnapshotDigest !== "string" || !/^[a-f0-9]{64}$/.test(registrySnapshotDigest)) {
      throw new Error("rightout_drop_registry_invalid");
    }
    return withProfile(cleanProfile, (profile, at) => {
      const brokerId = "ca_drop";
      const existing = profile.brokers[brokerId];
      if (existing && !["new", "action_selected", "request_rejected", "blocked"].includes(existing.state)) {
        throw new Error("rightout_drop_already_recorded");
      }
      const brokerCase = existing ?? newBrokerCase(brokerId, at);
      const schedule = calculateDropSchedule(at);
      ...
      if (schedule.phase_at_filing === "broker_processing_required") {
        transition(brokerCase, "awaiting_processing", at, "california_drop_processing_schedule");
      }
      const proof = opaqueEvidence("drop", [cleanProfile, registryCount, registrySnapshotDigest, schedule.contract_digest, at]);
      brokerCase.proof_references = [...new Set([...brokerCase.proof_references, proof])].slice(-24);
      ...
      brokerCase.next_recheck_at = schedule.first_status_check_at;
      brokerCase.coverage_gap = "portal_status_not_direct_deletion_proof_nonregistered_and_fcra_gaps_remain";
      brokerCase.mechanism = "california_drop";
      brokerCase.mechanism_status = "filed_human_verified";
      brokerCase.mechanism_contract_digest = schedule.contract_digest;
      brokerCase.mechanism_registry_scope = registryCount;
      brokerCase.mechanism_registry_snapshot_digest = registrySnapshotDigest;
      brokerCase.mechanism_processing_starts_at = schedule.processing_starts_at;
      brokerCase.mechanism_deadline_at = schedule.ordinary_deadline_at;
  ...
  async function recordDropStatus(profileId, observedStatus) {
    const cleanProfile = safeProfileId(profileId);
    if (!["pending", "deleted", "needs_manual_check"].includes(observedStatus)) {
      throw new Error("rightout_drop_status_invalid");
    }
    return withProfile(cleanProfile, (profile, at) => {
      const brokerCase = profile.brokers.ca_drop;
      if (
        !brokerCase || brokerCase.mechanism !== "california_drop"
        || brokerCase.mechanism_contract_digest !== dropContractDigest()
        || brokerCase.mechanism_deletion_confirmed !== false
        || brokerCase.mechanism_processing_starts_at !== DROP_CONTRACT.processing_starts_at
        || !Number.isInteger(brokerCase.mechanism_registry_scope)
        || brokerCase.mechanism_registry_scope < 1 || brokerCase.mechanism_registry_scope > 10_000
        || typeof brokerCase.mechanism_registry_snapshot_digest !== "string"
        || !/^[a-f0-9]{64}$/.test(brokerCase.mechanism_registry_snapshot_digest)
        || !["submitted", "awaiting_processing", "human_task_queued"].includes(brokerCase.state)
      ) throw new Error("rightout_drop_status_not_ready");
      let expectedSchedule;
      try {
        expectedSchedule = calculateDropSchedule(safeDate(brokerCase.submission_started_at, "drop_filed_at"));
      } catch {
        throw new Error("rightout_drop_status_not_ready");
      }
      if (
        expectedSchedule.contract_digest !== brokerCase.mechanism_contract_digest
        || expectedSchedule.processing_starts_at !== brokerCase.mechanism_processing_starts_at
        || expectedSchedule.ordinary_deadline_at !== brokerCase.mechanism_deadline_at
      ) throw new Error("rightout_drop_status_not_ready");
      const deadlineAt = expectedSchedule.ordinary_deadline_at;
      const processingStarted = Date.parse(at) >= Date.parse(DROP_CONTRACT.processing_starts_at);
      if (observedStatus === "deleted" && !processingStarted) throw new Error("rightout_drop_status_invalid");
      if (observedStatus === "needs_manual_check") {
        transition(brokerCase, "human_task_queued", at, "human_observed_drop_status_needs_manual_check");
        brokerCase.human_task_reason = "review_drop_portal_status_or_eligibility";
        brokerCase.next_recheck_at = null;
      } else {
        const deadlineElapsed = Date.parse(at) >= Date.parse(deadlineAt);
        const pendingPastDeadline = observedStatus === "pending" && deadlineElapsed;
        const target = pendingPastDeadline
          ? "human_task_queued"
          : processingStarted ? "awaiting_processing" : "submitted";
        transition(brokerCase, target, at, `human_observed_drop_portal_${observedStatus}`);
        brokerCase.human_task_reason = pendingPastDeadline ? "drop_ordinary_processing_deadline_elapsed" : null;
        const nextCycle = addDays(at, DROP_CONTRACT.broker_access_cycle_days);
        brokerCase.next_recheck_at = pendingPastDeadline
          ? null
          : observedStatus === "pending" && !processingStarted
            ? DROP_CONTRACT.processing_starts_at
            : observedStatus === "pending"
              ? new Date(Math.min(Date.parse(nextCycle), Date.parse(deadlineAt))).toISOString()
              : nextCycle;
      }
      const proof = opaqueEvidence("drop", [cleanProfile, observedStatus, brokerCase.mechanism_contract_digest, at]);
      brokerCase.proof_references = [...new Set([...brokerCase.proof_references, proof])].slice(-24);
      brokerCase.mechanism_status = `human_observed_${observedStatus}`;
      brokerCase.submission_outcome = observedStatus === "deleted"
        ? "human_observed_drop_portal_deleted_claim_not_deletion_proof"
        : `human_observed_drop_portal_${observedStatus}`;
      brokerCase.coverage_gap = "drop_portal_status_is_not_direct_record_level_deletion_proof";
      brokerCase.mechanism_deletion_confirmed = false;
      brokerCase.removal_confirmed_at = null;
      brokerCase.removal_confirmation_scope = null;
      profile.brokers.ca_drop = brokerCase;
      return {
        state: brokerCase.state,
        observed_status: observedStatus,
        proof_reference: proof,
        next_recheck_at: brokerCase.next_recheck_at,
        mechanism_status: brokerCase.mechanism_status,
        deletion_confirmed: false,
  ...
  async function verificationContext(profileId, brokerId, allowedStates = ["submitted", "verification_pending"]) {
    ...
    if ((!submitted && !identityPortalRequest) || !submissionProof) throw new Error("rightout_verification_case_not_ready");
    return {
      state: brokerCase.state,
  ...
  async function plan(profileId, catalog) {
    ...
    const rows = catalogRows(catalog).map((broker) => {
      ...
      return {
        ...
        ...(officialActionUrl ? {
          ...
          } : broker.uk_process && typeof broker.uk_process === "object" ? {
            effect_scope: safeMetadataToken(broker.uk_process.effect_scope),
            erasure_semantics: safeMetadataToken(broker.uk_process.erasure_semantics),
            rights_basis: safeMetadataToken(broker.uk_process.rights_basis),
            deadline_policy: safeMetadataToken(broker.uk_process.deadline_policy),
            identity_policy: safeMetadataToken(broker.uk_process.identity_policy),
            one_click_level: safeMetadataToken(broker.uk_process.one_click_level, "none"),
    ...
    const effectiveClusterChildren = rows.filter((row) => {
      if (row.cluster_role !== "child" || row.cluster_coverage_policy !== "official_registry_claims_one_site_request_applies_across_cluster") return false;
    ...
    return {
      ...
      summary: {
        ...
        uk_processes: rows.filter((row) => row.process_class.startsWith("uk_")).length,
  ...
  async function status(profileId) {
    ...
    const cases = Object.values(profile.brokers).sort((a, b) => a.broker_id.localeCompare(b.broker_id)).map((item) => ({
      ...
      ...(item.mechanism ? {
        ...
        mechanism_status: item.mechanism_status ?? null,
        mechanism_contract_digest: item.mechanism_contract_digest ?? null,
        mechanism_registry_scope: item.mechanism_registry_scope ?? null,
        mechanism_registry_snapshot_digest: item.mechanism_registry_snapshot_digest ?? null,
        mechanism_processing_starts_at: item.mechanism_processing_starts_at ?? null,
        mechanism_deadline_at: item.mechanism_deadline_at ?? null,
        mechanism_deletion_confirmed: item.mechanism_deletion_confirmed === true,
      } : {}),
    }));
    return {
      report_version: 1,

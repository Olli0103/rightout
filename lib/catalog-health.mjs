...
function catalogRows(catalog) {
  if (!catalog || !Array.isArray(catalog.brokers)) throw new Error("rightout_catalog_invalid");
...
export function assertFreshCatalogEntries(catalog, brokerIds, { now = Date.now() } = {}) {
  ...
  if (!health.live_provider_io_allowed) throw new Error("rightout_catalog_lane_stale");
  for (const brokerId of brokerIds) {
    const entry = rows.find((row) => row?.id === brokerId);
    if (!entry) throw new Error("rightout_catalog_lane_stale");
  }
}

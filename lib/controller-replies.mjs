...
function exactStrings(values, pattern, max) {
  ...
  if (out.length !== values.length) throw new Error("rightout_controller_reply_attestation_required");
  return out;
}

export function validateControllerReplyAttestations(input, value) {
  const cleanInput = publicInput(input);
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("rightout_controller_reply_attestation_required");
  ...
  if (
    digestKeys.length !== authorizedProfileIds.length
...
function controllerBroker(catalog, brokerId) {
  ...
  if (
    !broker || broker.removal?.supported !== true || broker.removal?.channel !== "email"
...
export function validateControllerReplyPreflight({ input, catalog, profilePayload, imapTransport, attestations }) {
  ...
  if (imapTransportDigest(imap) !== cleanAttestations.imapTransportDigest) throw new Error("rightout_controller_reply_snapshot_changed");
...
export function classifyControllerReply({ text, processClass }) {
  if (!["eu_controller_email_erasure", "uk_controller_email_erasure", "us_data_broker_email_deletion"].includes(processClass)) throw new Error("rightout_controller_reply_lane_unsupported");

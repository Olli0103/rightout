...
function cleanDomain(value) {
  if (typeof value !== "string") throw new Error("rightout_custom_target_invalid");
...
function cleanUrl(value, domain) {
  let url;
  try { url = new URL(value); } catch { throw new Error("rightout_custom_target_invalid"); }
...
function cleanPermission(value, handle, recipe, now) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("rightout_custom_target_permission_required");
  ...
  if (
    !Number.isFinite(reviewedAt) || !Number.isFinite(validUntil) || reviewedAt > now
...
export function createCustomTargetVault(store, { now = () => Date.now(), randomHandle = () => `custom_${randomBytes(12).toString("hex")}` } = {}) {
  if (!store || typeof store.registerIfAbsent !== "function" || typeof store.lookup !== "function") throw new Error("rightout_custom_target_store_invalid");

  async function intake({ profileId, actionUrl, sourceUrl, officialDomain, method }) {
    if (!SAFE_PROFILE_ID.test(profileId ?? "") || !METHODS.has(method)) throw new Error("rightout_custom_target_invalid");
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      if (!SAFE_HANDLE.test(handle)) throw new Error("rightout_custom_target_handle_invalid");
      const record = {
        schemaVersion: 1,
        handle,
        profileId,
  ...
  async function metadata(handle, profileId) {
    if (!SAFE_HANDLE.test(handle ?? "") || !SAFE_PROFILE_ID.test(profileId ?? "")) throw new Error("rightout_custom_target_ref_invalid");
    const record = validateStored(await store.lookup(handle), handle);
    if (record.profileId !== profileId) throw new Error("rightout_custom_target_scope_mismatch");
    return {
  ...
  async function resolveAuthorized(handle, profileId, { recipePacks, trustedKeys, permission }) {
    ...
    const record = validateStored(await store.lookup(handle), handle);
    if (!Array.isArray(recipePacks) || recipePacks.length < 1 || recipePacks.length > 20) throw new Error("rightout_custom_target_recipe_required");
    const recipes = recipePacks.flatMap((pack) => verifyExternalRecipePack(pack, trustedKeys, { now: now() }).recipes);
    const matches = recipes.filter((recipe) => (
      recipe.method === record.method
    ...
    if (matches.length !== 1) throw new Error("rightout_custom_target_recipe_required");

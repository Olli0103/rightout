...
export function calculateDropSchedule(filedAt) {
  ...
  const firstStatusCheck = filed < processingStart
    ? processingStart
    : new Date(Math.min(
      filed.getTime() + DROP_CONTRACT.broker_access_cycle_days * DAY_MS,
      ordinaryDeadline.getTime(),
    ));
  return {
...
export function dropRegistrySnapshot(value, { now = Date.now() } = {}) {
  if (!Number.isFinite(now)) throw new Error("rightout_drop_registry_invalid");
...
export function dropStatusScopeBinding(value, caseContext) {
  ...
  if (
    !caseContext || typeof caseContext !== "object" || Array.isArray(caseContext)

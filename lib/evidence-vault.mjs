  ...
  async function cleanupExpiredExports() {
    ...
    for (const entry of await exportStore.entries()) {
      ...
      if (Date.parse(record.expiresAt) <= current) {
        if (await unlink(join(directory, record.artifactName)).then(() => true).catch(() => false)) deleted += 1;
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      if (SAFE_EXPORT_TEMP_NAME.test(entry.name)) {
        if (await unlink(join(directory, entry.name)).then(() => true).catch(() => false)) orphaned += 1;
        continue;
      }

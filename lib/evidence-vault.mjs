...
import { createStateTransactionLock } from "./state-transaction-lock.mjs";
...
/**
 * @param {any} store
 * @param {{ stateDir: string, now?: () => Date, exportStore?: any, exportRoot?: string, setTimer?: typeof setTimeout, clearTimer?: typeof clearTimeout, onCleanupError?: (error: unknown) => void }} options
 */
export function createEvidenceVault(store, options = {}) {
  const {
    stateDir, now = () => new Date(), exportStore, exportRoot,
    setTimer = setTimeout, clearTimer = clearTimeout, onCleanupError = () => undefined,
  } = options;
  if (typeof stateDir !== "string" || !stateDir) throw new Error("rightout_evidence_store_invalid");
  if (
    !store || typeof store.registerIfAbsent !== "function" || typeof store.lookup !== "function"
  ...
  if ((exportStore === undefined) !== (exportRoot === undefined)) throw new Error("rightout_evidence_export_store_invalid");
  if (exportStore && (
    typeof exportStore.register !== "function" || typeof exportStore.lookup !== "function"
  ...
  let cleanupTimer;
  const transactionLock = createStateTransactionLock({ stateDir, namespace: "rightout-evidence-transaction-v1" });

  /** @template T @param {() => Promise<T> | T} operation @returns {Promise<T>} */
  function serialized(operation) {
    return transactionLock.run(operation);
  }

  function armCleanupTimer(delay) {
    cleanupTimer = setTimer(async () => {
      cleanupTimer = undefined;
      try { await serialized(cleanupExpiredEvidenceInternal); }
      catch (error) {
  ...
  async function tightenExportExpiry(ref, expiresAt) {
    ...
    for (const entry of await exportStore.entries()) {
      const record = validateExportRecord(entry.value, entry.key);
      if (record.evidenceRef !== ref || Date.parse(record.expiresAt) <= Date.parse(expiresAt)) continue;
  ...
  async function cleanupExpiredExportsInternal() {
    if (!exportStore || exportRoot === undefined) return { deleted: 0, orphaned: 0 };
    ...
    for (const entry of await exportStore.entries()) {
      ...
      if (Date.parse(record.expiresAt) <= current) {
        if (await unlinkManagedArtifact(join(directory, record.artifactName), "rightout_evidence_export_cleanup_failed")) deleted += 1;
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      if (SAFE_EXPORT_TEMP_NAME.test(entry.name)) {
        if (await unlinkManagedArtifact(join(directory, entry.name), "rightout_evidence_export_cleanup_failed")) orphaned += 1;
        continue;
      }
      if (!SAFE_EXPORT_NAME.test(entry.name) || liveNames.has(entry.name)) continue;
      if (await unlinkManagedArtifact(join(directory, entry.name), "rightout_evidence_export_cleanup_failed")) orphaned += 1;
    }
  ...
  async function exportRedactedInternal(ref, profileId, root, format = "json") {
    if (exportRoot !== undefined && root !== exportRoot) throw new Error("rightout_evidence_export_root_invalid");
    ...
    if (exportStore) {
      ...
      try { await exportStore.register(key, exportRecord); }
      catch {
  ...
  async function purgeExportsInternal(profileId) {
    if (!SAFE_PROFILE_ID.test(profileId ?? "") || !exportStore || exportRoot === undefined) return 0;
    const directory = await privateExportDirectory(exportRoot);
    let deleted = 0;
    for (const entry of await exportStore.entries()) {
      const record = validateExportRecord(entry.value, entry.key);
  ...
  async function purgeSubjectInternal(profileId) {
    if (!SAFE_PROFILE_ID.test(profileId ?? "")) throw new Error("rightout_evidence_scope_invalid");

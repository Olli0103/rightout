...
function safePath(root, child) {
  ...
  if (!rel || rel.startsWith("..") || isAbsolute(rel)) throw new Error("rightout_state_path_invalid");
  return target;
}
...
async function ensurePrivateDirectory(stateDir) {
  ...
  try { await mkdir(dataDir, { mode: 0o700 }); }
  catch (error) { if (error?.code !== "EEXIST") throw new Error("rightout_state_directory_unsafe"); }
  const metadata = await lstat(dataDir);
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) throw new Error("rightout_state_directory_unsafe");
  await chmod(dataDir, 0o700);
  const canonical = await realpath(dataDir);
  if (canonical !== dataDir || dirname(canonical) !== trustedRoot) throw new Error("rightout_state_directory_unsafe");
  return dataDir;
}
...
function cleanKey(value) {
  if (typeof value !== "string" || !SAFE_KEY.test(value)) throw new Error("rightout_state_key_invalid");
  return value;
}

function cloneValue(value) {
  let text;
  try { text = JSON.stringify(value); } catch { throw new Error("rightout_state_value_invalid"); }
  if (text === undefined || Buffer.byteLength(text) > MAX_VALUE_BYTES) throw new Error("rightout_state_value_invalid");
...
function decryptState(bytes, secret, namespace) {
  let envelope;
  try { envelope = JSON.parse(bytes.toString("utf8")); } catch { throw new Error("rightout_state_corrupt"); }
  if (envelope?.schemaVersion !== 1) throw new Error("rightout_state_corrupt");
  try {
    ...
    if (error instanceof Error && error.message === "rightout_state_corrupt") throw error;
    throw new Error("rightout_state_decryption_failed");
  }
}

export function createEncryptedFileKeyedStore({
  stateDir,
  namespace,
  maxEntries,
  defaultTtlMs = /** @type {number | undefined} */ (undefined),
  getSecret,
  now = () => Date.now(),
  _testHooks = undefined,
}) {
  if (typeof stateDir !== "string" || !stateDir || !SAFE_NAMESPACE.test(namespace)) throw new Error("rightout_state_options_invalid");
  if (!Number.isInteger(maxEntries) || maxEntries < 1 || maxEntries > 2_000) throw new Error("rightout_state_options_invalid");
  if (defaultTtlMs !== undefined && (!Number.isInteger(defaultTtlMs) || defaultTtlMs < 1)) throw new Error("rightout_state_options_invalid");
  if (typeof getSecret !== "function") throw new Error("rightout_state_options_invalid");
  ...
  async function cleanupCreatedLock(lockDir, identity, token) {
    ...
    if (owner && (owner.pid !== process.pid || owner.token !== token)) return true;
  ...
  async function acquireFileLock() {
    ...
    for (let attempt = 0; attempt < 400; attempt += 1) {
      ...
      try {
        ...
        if (
          !sameIdentity(createdIdentity, currentIdentity)
          || !publishedOwner
        ...
        if (error?.code !== "EEXIST") throw new Error("rightout_state_lock_failed");
        const metadata = await lstat(lockDir).catch(() => undefined);
        if (!metadata || !metadata.isDirectory() || metadata.isSymbolicLink()) throw new Error("rightout_state_lock_unsafe");
        const owner = await readLockOwner(lockDir);
  ...
  async function readState(file, secret) {
    let handle;
    ...
    try {
      const metadata = await handle.stat();
      if (!metadata.isFile() || metadata.size > MAX_FILE_BYTES || (metadata.mode & 0o077) !== 0) throw new Error("rightout_state_file_unsafe");
      return decryptState(await handle.readFile(), secret, namespace);
    } finally { await handle.close().catch(() => undefined); }
  }

  function prune(state, at) {
    for (const [key, entry] of Object.entries(state.entries)) {
      if (!entry || typeof entry !== "object" || (entry.expiresAt !== undefined && entry.expiresAt <= at)) delete state.entries[key];
    }
  }

  async function writeState(directory, file, state, secret) {
    const bytes = encryptState(state, secret, namespace);
    if (bytes.length > MAX_FILE_BYTES) throw new Error("rightout_state_too_large");
  ...
  function ttl(opts) {
    ...
    if (value !== undefined && (!Number.isInteger(value) || value < 1)) throw new Error("rightout_state_ttl_invalid");
    return value;
  }
  ...
  async function mutate(callback) {
    return serialized(async () => {
      ...
      const state = await readState(file, secret);
      prune(state, now());
      const outcome = await callback(state);
      await writeState(directory, file, state, secret);
      return outcome;
    });
  }

  async function register(key, value, opts) {
    const clean = cleanKey(key); const copy = cloneValue(value); const ttlMs = ttl(opts);
    return mutate((state) => {
      const at = now();
      ...
      while (ordered.length > maxEntries) delete state.entries[ordered.shift()[0]];
    });
  }
  ...
  async function registerIfAbsent(key, value, opts) {
    const clean = cleanKey(key); const copy = cloneValue(value); const ttlMs = ttl(opts);
    return mutate((state) => {
      if (state.entries[clean]) return false;
      const at = now();
      state.entries[clean] = { value: copy, createdAt: at, ...(ttlMs ? { expiresAt: at + ttlMs } : {}) };
      const ordered = Object.entries(state.entries).sort((a, b) => a[1].createdAt - b[1].createdAt);
      while (ordered.length > maxEntries) delete state.entries[ordered.shift()[0]];
  ...
  async function lookup(key) {
    ...
    return serialized(async () => {
      const secret = getSecret(); encryptionKey(secret);
  ...
  async function entries() {
    return serialized(async () => {
      ...
      const before = Object.keys(state.entries).length;
      prune(state, now());
      if (Object.keys(state.entries).length !== before) await writeState(directory, file, state, secret);
  ...
  async function update(key, updateValue, opts) {
    const clean = cleanKey(key); const ttlMs = ttl(opts);
    if (typeof updateValue !== "function") throw new Error("rightout_state_update_invalid");
    return mutate((state) => {
      const next = updateValue(state.entries[clean] ? cloneValue(state.entries[clean].value) : undefined);
      if (next === undefined) { const existed = Boolean(state.entries[clean]); delete state.entries[clean]; return existed; }
      const at = now();
  ...
  return { register, registerIfAbsent, update, lookup, consume, delete: deleteKey, entries, clear };
}

export const __test = { safePath, encryptionKey, encryptState, decryptState };

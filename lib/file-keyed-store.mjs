...
function decryptStateFromRing(bytes, secrets, namespace) {
  ...
  for (const secret of secrets) {
    try { return decryptState(bytes, secret, namespace); }
    catch (error) {
      if (error?.message === "rightout_state_corrupt") throw error;
      lastError = error;
    }
  }
  if (lastError) throw new Error("rightout_state_decryption_failed");
  throw new Error("rightout_state_encryption_key_required");
}
...
export function createEncryptedFileKeyedStore({
  ...
  getSecret,
  getPreviousSecrets = /** @type {() => string[]} */ (() => []),
  now = () => Date.now(),
  ...
  if (typeof getPreviousSecrets !== "function") throw new Error("rightout_state_options_invalid");
  ...
  async function entries() {
    return serialized(async () => {
      ...
      const before = Object.keys(state.entries).length;
      prune(state, now());
      if (Object.keys(state.entries).length !== before) await writeState(directory, file, state, keys.active);

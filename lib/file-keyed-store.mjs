...
export function createEncryptedFileKeyedStore({
  ...
  async function acquireFileLock() {
    ...
    for (let attempt = 0; attempt < 400; attempt += 1) {
      ...
      try {
        ...
        let metadata;
        try { metadata = await (_testHooks?.lstatLockDir ?? lstat)(lockDir); }
        catch (metadataError) {
          if (metadataError?.code !== "ENOENT") throw new Error("rightout_state_lock_unsafe");
          await new Promise((resolveWait) => setTimeout(resolveWait, 25));
          continue;
        }
        if (!metadata.isDirectory() || metadata.isSymbolicLink()) throw new Error("rightout_state_lock_unsafe");
        const owner = await readLockOwner(lockDir);
  ...
  async function entries() {
    return serialized(async () => {
      ...
      const state = await readState(file, keys.all);
      const changed = prune(state, now());
      if (changed) await writeState(directory, file, state, keys.active);

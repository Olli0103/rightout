...
export async function runFormRemoval({
  input, catalog, profilePayload, attestations, bridgeUrl, browserProfile, browserAuthToken,
  browserBackend, browserControlTransport, approvalBoundary = "assisted_allow_once",
  submitForm, signal, now = () => new Date(),
}) {
  ...
  if (!["managed_openclaw", "remote_cloud_cdp", "existing_logged_in_cdp"].includes(browserBackend)) throw new Error("rightout_browser_backend_invalid");
  if (!["openclaw_sandbox_browser_bridge", "standalone_loopback_http_opt_in"].includes(browserControlTransport)) throw new Error("rightout_browser_backend_invalid");

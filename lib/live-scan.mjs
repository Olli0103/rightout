...
const BRAVE_COUNTRY_TARGETS = new Set((
  "AR AU AT BE BR CA CL DK FI FR DE HK IN ID IT JP KR MY MX NL NZ NO CN PL PT PH RU ZA ES SE CH TW TR GB US"
).split(" "));
const COUNTRY_SEARCH_LANG = Object.freeze({
  AR: "es", AT: "de", AU: "en", BE: "nl", BR: "pt", CA: "en", CH: "de", CL: "es", CN: "zh-hans",
  DE: "de", DK: "da", ES: "es", FI: "fi", FR: "fr", GB: "en", HK: "zh-hant", ID: "id", IN: "en",
  IT: "it", JP: "jp", KR: "ko", MX: "es", MY: "en", NL: "nl", NO: "no", NZ: "en", PH: "en",
  PL: "pl", PT: "pt-pt", RU: "ru", SE: "sv", TR: "tr", TW: "zh-hant", US: "en",
});
...
function cleanScanConsent(value) {
  ...
  const method = value.method === undefined ? "self" : cleanInput(value.method, "consent_method", 3, 24);
  if (!new Set(["self", "written_authorization", "poa"]).has(method)) throw new Error("subject_consent_required");
...
function parseSubjectProfile(value) {
  ...
  const allowedKeys = new Set(["fullName", "city", "region", "country", "contactEmail", "jurisdictions", "mobileAdvertisingId", "dateOfBirth", "alsoKnownAs", "emails", "phones", "priorLocations", "currentAddress", "priorAddresses", "consent"]);
  ...
  if (!ISO_COUNTRIES.has(country)) throw new Error("unsupported_country");
...
function cleanAddress(value, defaultCountry = "US") {
  ...
  const country = cleanInput(value.country ?? defaultCountry, "address_country", 2, 2).toUpperCase();
  if (!ISO_COUNTRIES.has(country)) throw new Error("unsupported_country");
...
function cleanPriorLocations(values, defaultCountry = "US") {
  ...
  const out = values.map((value) => {
    ...
    const country = cleanInput(value.country ?? defaultCountry, "prior_country", 2, 2).toUpperCase();
    if (!ISO_COUNTRIES.has(country)) throw new Error("unsupported_country");
...
function braveQueryWithinLimits(query) {
  return query.length <= BRAVE_MAX_QUERY_CHARACTERS
    && query.trim().split(/\s+/u).filter(Boolean).length <= BRAVE_MAX_QUERY_WORDS;
}

function boundedBraveQuery(officialDomain, requiredTerm, optionalTerms = []) {
  let query = `site:${officialDomain} "${requiredTerm}"`;
  if (!braveQueryWithinLimits(query)) return null;
  for (const term of optionalTerms) {
    const candidate = `${query} "${term}"`;
    if (braveQueryWithinLimits(candidate)) query = candidate;
  }
  return query;
}
...
export function buildSearchVectors(subject, officialDomain) {
  ...
  for (const name of names) {
    for (const location of locations) {
      vectors.push({ kind: "name_location", query: boundedBraveQuery(officialDomain, name, [location.city, location.region]) });
    }
  }
  for (const email of [subject.contactEmail, ...(subject.emails ?? [])].filter(Boolean)) {
    vectors.push({ kind: "email", query: boundedBraveQuery(officialDomain, email) });
  }
  for (const phone of subject.phones ?? []) vectors.push({ kind: "phone", query: boundedBraveQuery(officialDomain, phone) });
  for (const address of [subject.currentAddress, ...(subject.priorAddresses ?? [])].filter(Boolean)) {
    vectors.push({ kind: "address", query: boundedBraveQuery(officialDomain, address.line1, [address.city, address.region, address.postal]) });
  }
  const unique = [];
  const seen = new Set();
  let omitted = 0;
...
export function approvalDescription(input) {
  ...
  const explicit = validated.brokerIds.join(",");
  const digest = createHash("sha256").update(validated.brokerIds.slice().sort().join("\n")).digest("hex").slice(0, 12);
  const brokers = explicit.length <= 80 ? explicit : `${validated.brokerIds.length}@${digest}`;
  return `P ${validated.profileId}; B ${brokers}. Brave index (terms ${BRAVE_TERMS_VERSION}; consent+duties; logs <=90d/ZDR). Configured names/aliases/addresses/emails/phones. No broker request/email/write.`;
}

function braveLocaleForCountry(country) {
  return {
    country: BRAVE_COUNTRY_TARGETS.has(country) ? country : "ALL",
...
export async function runLiveScan({
  ...
  approvalBoundary = "assisted_allow_once",
}) {
  throwIfAborted(signal);
  ...
  if (!["assisted_allow_once", "finite_campaign_grant"].includes(approvalBoundary)) {
    throw new Error("rightout_scan_approval_boundary_invalid");
  }
  const subject = validated.subject;
  const braveLocale = braveLocaleForCountry(subject.country);
  ...
  const results = await mapBounded(selected, 4, async (broker) => {
      throwIfAborted(signal);
      const officialDomains = cleanOfficialDomains(broker.official_domains);
      const { vectors, omitted: vectorsOmitted } = buildSearchVectors(subject, officialDomains[0]);
      let candidate = false;
      let candidateUrls = [];
      let attempted = 0;
      let failure;
      for (const vector of vectors) {
        try {
          const payload = await guardedJsonPost(
            guardedFetch,
            BRAVE_ENDPOINT,
            { q: vector.query, country: braveLocale.country, search_lang: braveLocale.search_lang, safesearch: "strict", count: 10 },
            apiKey,
            signal,
          );
          attempted += 1;
      ...
      return candidate ? {
        ...
        vectors_omitted_for_provider_limits: vectorsOmitted,
      } : {
        broker_id: broker.id,
        state: "inconclusive",
        proof_references: [],
        reason: failure ?? (vectorsOmitted > 0
          ? "query_scope_partially_or_fully_exceeds_brave_limits"
          : "no_index_candidates_not_proof_of_absence"),
        vectors_attempted: attempted,
        vector_types: [...new Set(vectors.slice(0, attempted).map((vector) => vector.kind))],
        vectors_omitted_for_provider_limits: vectorsOmitted,
      };
  ...
  return {
    ...
    mode: approvalBoundary === "finite_campaign_grant" ? "campaign_gated_live_scan" : "approval_gated_live_scan",
    ...
    provider: {
      ...
      broker_parallelism: Math.min(4, selected.length),
      locale_targeting: braveLocale.localization,

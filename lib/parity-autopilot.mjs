...
export function planGlobalScanCampaignNext({ campaign, caseStatus, scanCatalog }) {
  ...
  const eligible = new Set(rows.filter((row) => (
    row && ["people_search", "data_broker"].includes(row.category)
    && row.scan?.supported === true
    && row.scan?.automated_access_policy === "search_index_only_no_publisher_access"
  )).map((row) => row.id));
...
export function planParityCampaignNext({
  ...
  if (!["imap", "unavailable"].includes(verificationMode)) throw new Error("rightout_autopilot_transport_invalid");

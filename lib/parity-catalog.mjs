...
function uniqueStrings(values, pattern, { min = 1, max = 20 } = {}) {
  if (!Array.isArray(values) || values.length < min || values.length > max) throw new Error("rightout_parity_catalog_invalid");
  if (values.some((value) => typeof value !== "string" || !pattern.test(value))) throw new Error("rightout_parity_catalog_invalid");
  if (new Set(values).size !== values.length) throw new Error("rightout_parity_catalog_invalid");
  return [...values];
}
...
function cleanRoute(value) {
  ...
  if (!new Set(["web_form", "email", "phone"]).has(value.method)) throw new Error("rightout_parity_catalog_invalid");
  ...
  if (
    !new Set(["web_form", "email", "phone"]).has(reference.method)
  ...
  if (
    JSON.stringify(executionJurisdictions) !== JSON.stringify(["US", "US-CA"])
  ...
  if (fields.some((field) => !SAFE_FIELDS.has(field))) throw new Error("rightout_parity_catalog_invalid");
  let actionUrl;
  let sourceUrl;
  try { actionUrl = new URL(value.action_url); sourceUrl = new URL(value.source_url); }
  catch { throw new Error("rightout_parity_catalog_invalid"); }
  ...
  let referenceUrl;
  try { referenceUrl = new URL(reference.action_url); } catch { throw new Error("rightout_parity_catalog_invalid"); }
  if (
    referenceUrl.protocol !== "https:" || referenceUrl.username || referenceUrl.password
  ...
  if (mappedReferenceInputs.some((field) => !fields.includes(field))) throw new Error("rightout_parity_catalog_invalid");
  if (value.id === "rehold") {
    ...
    if (
      current.method !== value.method || current.action_url !== value.action_url
    ...
    if (expectedEvidence.size !== 0) throw new Error("rightout_parity_catalog_invalid");
  } else if (value.current_contract !== undefined) throw new Error("rightout_parity_catalog_invalid");
  ...
  if (EXTERNAL_UNAVAILABLE_STATUS.has(value.source_status)) {
    let sourceEvidence;
    try { sourceEvidence = new URL(value.source_evidence_url); } catch { throw new Error("rightout_parity_catalog_invalid"); }
    ...
    if (
      sourceEvidence.protocol !== "https:" || sourceEvidence.username || sourceEvidence.password
  ...
  if (value.cluster_parent !== undefined && !SAFE_ID.test(value.cluster_parent)) throw new Error("rightout_parity_catalog_invalid");
  if (value.rescue_email !== undefined) {
    if (typeof value.rescue_email !== "string" || !/^[^@\s]{1,64}@[a-z0-9.-]{3,253}$/.test(value.rescue_email)) {
      throw new Error("rightout_parity_catalog_invalid");
    }
    let rescueSource;
    try { rescueSource = new URL(value.rescue_source_url); } catch { throw new Error("rightout_parity_catalog_invalid"); }
    const officialRegistry = (
      rescueSource.hostname === "cppa.ca.gov" && /^\/data_broker_registry\/registry\d{4}\.csv$/.test(rescueSource.pathname)
    ) || (
      rescueSource.hostname === "oag.ca.gov" && rescueSource.pathname === "/node/550652"
    );
    ...
    if (
      (value.rescue_source_status === "observed_official_registry" && !officialRegistry)
  ...
  ].some((item) => item !== undefined)) throw new Error("rightout_parity_catalog_invalid");
...
export function validateParityCatalog(value) {
  ...
  if (Object.values(value.policy).some((item) => item !== true)) throw new Error("rightout_parity_catalog_invalid");
  if (!Array.isArray(value.brokers)) throw new Error("rightout_parity_catalog_invalid");
  const brokers = value.brokers.map(cleanRoute);
  const ids = brokers.map((broker) => broker.id).sort();
  if (JSON.stringify(ids) !== JSON.stringify([...EXPECTED_IDS].sort())) throw new Error("rightout_parity_catalog_invalid");
  ...
  if (methods.web_form !== 20 || methods.email !== 1 || methods.phone !== 1) throw new Error("rightout_parity_catalog_invalid");
...
export function assertParityCatalogFresh(catalog, { now = Date.now(), maxAgeDays = 180 } = {}) {
  ...
  if (!health.release_ready) throw new Error("rightout_catalog_lane_stale");
...
export function resolveParityBroker(catalog, brokerId) {
  if (typeof brokerId !== "string" || !SAFE_ID.test(brokerId)) throw new Error("rightout_parity_broker_invalid");
  const clean = validateParityCatalog(catalog);
  const broker = clean.brokers.find((row) => row.id === brokerId);
  if (!broker) throw new Error("rightout_parity_broker_invalid");
  return broker;
}

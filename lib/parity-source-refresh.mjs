...
function classify(route, status) {
  if (route.source_status === "needs_evidence") return "needs_human_route_review";
  if (route.source_status === EXTERNAL_UNAVAILABLE_STATUS) {
    if (status === 404 || status === 410 || status >= 500) return "external_unavailable_reconfirmed";
    return "external_route_recovered_needs_catalog_review";
  }
  if (status >= 200 && status < 300) return "official_endpoint_observed";
  if ([401, 403, 405, 429].includes(status)) return "official_endpoint_access_control_observed";
  if (status >= 300 && status < 400) return "redirect_quarantined";
...
export async function refreshParitySources({ catalog, guardedFetch, permissionForRoute, signal, now = () => new Date() }) {
  ...
  if (typeof guardedFetch !== "function") throw new Error("rightout_parity_source_refresh_unavailable");
  if (typeof permissionForRoute !== "function") throw new Error("rightout_parity_source_permission_contract_required");
  const routes = [...catalog.brokers].sort((a, b) => a.id.localeCompare(b.id));
  const results = await mapBounded(routes, 4, async (route) => {
    if (signal?.aborted) throw new Error("rightout_parity_source_refresh_cancelled");
    let permission;
    try { permission = permissionForRoute(route); }
    catch { permission = null; }
    if (!permission) return { broker_id: route.id, state: "not_probed_permission_required", network_requests: 0 };
    let request;
    try {
      ...
      if (!Number.isInteger(status) || status < 100 || status > 599) throw new Error("invalid_status");

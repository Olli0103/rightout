...
export function canonicalRecipeJson(value) {
  let text;
  try { text = JSON.stringify(stableValue(value)); } catch { throw new Error("rightout_recipe_invalid"); }
  if (text === undefined || Buffer.byteLength(text) > 2_000_000) throw new Error("rightout_recipe_invalid");
  return text;
}
...
function cleanDomains(values) {
  if (!Array.isArray(values) || values.length < 1 || values.length > 12) throw new Error("rightout_recipe_invalid");
  ...
  if (new Set(domains).size !== domains.length) throw new Error("rightout_recipe_invalid");
  return [...domains].sort();
}
...
function cleanFields(values) {
  ...
  if (new Set(values).size !== values.length) throw new Error("rightout_recipe_invalid");
  return [...values];
}

function safeHttpsUrl(value, domains) {
  let url;
  try { url = new URL(value); } catch { throw new Error("rightout_recipe_invalid"); }
  if (
...
function cleanSteps(steps, method) {
  if (!Array.isArray(steps) || steps.length < 1 || steps.length > 16) throw new Error("rightout_recipe_invalid");
  ...
  return steps.map((step) => {
    if (!step || typeof step !== "object" || Array.isArray(step)) throw new Error("rightout_recipe_invalid");
    if (Object.keys(step).some((key) => !["id", "kind", "required_fields", "success_markers"].includes(key))) throw new Error("rightout_recipe_invalid");
    if (typeof step.id !== "string" || !/^[a-z][a-z0-9_]{2,80}$/.test(step.id) || ids.has(step.id)) throw new Error("rightout_recipe_invalid");
    ids.add(step.id);
    if (!allowedKinds.has(step.kind)) throw new Error("rightout_recipe_invalid");
...
export function validateRecipe(recipe) {
  if (!recipe || typeof recipe !== "object" || Array.isArray(recipe)) throw new Error("rightout_recipe_invalid");
  ...
  if (Object.keys(recipe).some((key) => !allowed.has(key)) || recipe.schema_version !== 1) throw new Error("rightout_recipe_invalid");
  if (
    typeof recipe.recipe_id !== "string" || !/^recipe_[a-z0-9_]{2,80}_v[1-9][0-9]*$/.test(recipe.recipe_id)
  ...
  if (!Number.isFinite(reviewedAt) || !Number.isFinite(expiresAt) || expiresAt <= reviewedAt) throw new Error("rightout_recipe_invalid");
...
export function buildBuiltinRecipes(catalog, { expiresAt = "2027-07-14" } = {}) {
  if (!catalog || typeof catalog !== "object" || !Array.isArray(catalog.brokers) || catalog.brokers.length < 1) throw new Error("rightout_recipe_catalog_invalid");
  const recipes = catalog.brokers.map((row) => {
    if (
      !row || !SAFE_BROKER_ID.test(row.id ?? "") || !METHODS.has(row.method)
    ...
    const sourceContractDigest = recipeDigest({
      ...
      execution_jurisdictions: row.execution_jurisdictions,
      execution_market_ids: row.execution_market_ids,
      provider_request_contract: row.provider_request_contract,
  ...
  if (new Set(recipes.map((recipe) => recipe.broker_id)).size !== recipes.length) throw new Error("rightout_recipe_catalog_invalid");
  return recipes;
}
...
export function compileBuiltinRecipePack(catalog, manifest, { sourceSha256 = "", now = Date.now() } = {}) {
  ...
  if (!Number.isFinite(expires) || expires <= now) throw new Error("rightout_recipe_pack_expired");
  const recipes = buildBuiltinRecipes(catalog, { expiresAt: manifest.expires_at });
  if (recipeDigest(recipes) !== manifest.compiled_recipes_sha256) throw new Error("rightout_recipe_pack_integrity_failed");
  return {
    schema_version: 1,
...
export function verifyExternalRecipePack(pack, trustedKeys, { now = Date.now() } = {}) {
  if (!pack || typeof pack !== "object" || Array.isArray(pack)) throw new Error("rightout_recipe_pack_invalid");
  const allowed = new Set(["schema_version", "pack_id", "trust", "key_id", "issued_at", "expires_at", "recipes", "signature"]);
  if (
    Object.keys(pack).some((key) => !allowed.has(key)) || pack.schema_version !== 1 || pack.trust !== "external_ed25519"
  ...
  if (!Array.isArray(pack.recipes) || pack.recipes.length < 1 || pack.recipes.length > 500) throw new Error("rightout_recipe_pack_invalid");
  const recipes = pack.recipes.map(validateRecipe);
  if (new Set(recipes.map((row) => row.broker_id)).size !== recipes.length) throw new Error("rightout_recipe_pack_invalid");
  ...
  let key;
  try { key = createPublicKey(trustedKeys[pack.key_id]); } catch { throw new Error("rightout_recipe_pack_key_invalid"); }
  if (key.asymmetricKeyType !== "ed25519") throw new Error("rightout_recipe_pack_key_invalid");
  let signature;
  try { signature = Buffer.from(pack.signature, "base64url"); } catch { throw new Error("rightout_recipe_pack_signature_invalid"); }
  if (signature.byteLength !== 64) throw new Error("rightout_recipe_pack_signature_invalid");
  if (!verifySignature(null, Buffer.from(canonicalRecipeJson(unsigned)), key, signature)) throw new Error("rightout_recipe_pack_signature_invalid");
  return {
    schema_version: 1,
...
export function assessRecipeSnapshot(recipeInput, snapshot) {
  ...
  const hasKnownAction = names.some((name) => (
    /\b(?:search|continue|submission|confirmation|suppression|consent|send) action\b/u.test(name)

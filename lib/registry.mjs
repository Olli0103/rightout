...
function parseCsv(text) {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (quoted) {
      ...
      if (row.length > MAX_COLUMNS) throw new Error("rightout_registry_csv_invalid");
    } else if (char === "\n" || char === "\r") {
      if (char === "\r" && text[index + 1] === "\n") index += 1;
      row.push(field); field = "";
      rows.push(row); row = [];
      if (rows.length > MAX_ROWS) throw new Error("rightout_registry_csv_invalid");
    } else field += char;
    if (field.length > MAX_FIELD_CHARS) throw new Error("rightout_registry_csv_invalid");
  }
  if (quoted) throw new Error("rightout_registry_csv_invalid");
...
function columnMap(rows) {
  ...
  if (headerIndex < 0) throw new Error("rightout_registry_schema_invalid");
  ...
  if (Object.values(columns).some((index) => index < 0)) throw new Error("rightout_registry_schema_invalid");
...
function safeHttps(value) {
  ...
  try {
    ...
    if (url.protocol !== "https:" || url.username || url.password || !SAFE_DOMAIN.test(url.hostname)) return null;
    url.hash = "";
    return url.toString();
  } catch { return null; }
}
...
export function parseCaliforniaRegistryCsv(text, { sourceUrl = "", retrievedAt = new Date().toISOString() } = {}) {
  ...
  for (const row of rows.slice(headerIndex + 1)) {
    ...
    if (seen.has(brokerRef)) continue;
    seen.add(brokerRef);
    records.push({
  ...
  if (records.length < 100) throw new Error("rightout_registry_record_count_invalid");
  return {
    schema_version: 1,
    jurisdiction: "US-CA",
...
export async function readBoundedText(response) {
  ...
  if (!response?.body || declared > MAX_CSV_BYTES) throw new Error("rightout_registry_download_invalid");
  ...
  try {
    while (true) {
      ...
      if (size > MAX_CSV_BYTES) throw new Error("rightout_registry_download_invalid");
...
export function registrySummary(snapshot) {
  if (!snapshot || snapshot.schema_version !== 1 || !Array.isArray(snapshot.records)) throw new Error("rightout_registry_state_invalid");
  return {
    schema_version: 1,

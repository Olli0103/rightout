...
export function parseRemovalProfile(value) {
  ...
  if (!/^[A-Z0-9][A-Z0-9 .'-]{1,39}$/.test(region) || !ISO_COUNTRIES.has(country)) throw new Error("profile_invalid");
  ...
  if (mobileAdvertisingId !== undefined && !SAFE_MOBILE_ADVERTISING_ID.test(mobileAdvertisingId)) throw new Error("profile_invalid");
...
function cleanPriorLocations(values, defaultCountry) {
  ...
  const out = values.map((location) => {
    ...
    if (!ISO_COUNTRIES.has(country)) throw new Error("profile_invalid");
...
function cleanRemovalEntry(catalog, input) {
  ...
  if (
    (input.requestKind === "delete_and_opt_out" && templateId !== "us_delete_opt_out_v1")
  ...
  if (
    (broker.category === "people_search" && discoveryRequirement !== "prior_discovery_required")
  ...
  if (
    (isPeopleSearch && (
      input.requestKind !== "delete_and_opt_out"
    ...
    || (isUkController && (
      input.requestKind !== UK_RIGHTS_CONTRACT.request_kind
      || removal.confirmation_policy !== "submitted_until_controller_response"
    ))
    || (isUsDataBroker && (
      input.requestKind !== "delete_and_opt_out"
  ...
  if (
    !Number.isInteger(processingDays)
  ...
  if (
    (isPeopleSearch && eligibleJurisdictions.join(",") !== "US-CA")
...
export function validateSmtpConfig(value, profile) {
  ...
  if (authMode === "oauth2") {
    if (value.password !== undefined) throw new Error("rightout_smtp_not_configured");
  ...
  if (value.authMode === "password") return { host, port, secure, username, authMode, password, fromAddress };
  return { host, port, secure, username, password, fromAddress };
}

function cleanOauthExpiry(value, protocol) {
  if (typeof value !== "string" || value.length < 20 || value.length > 35) throw new Error(`rightout_${protocol}_oauth_expired`);
...
function assertEligible(profile, broker) {
  ...
  if (
    broker.eligibleJurisdictions.some((jurisdiction) => jurisdiction === "EU" || jurisdiction === "EEA")
    && (
  ...
  if (
    broker.eligibleJurisdictions.includes("UK")
    && (
      profile.country !== "GB"
...
function disclosureLines(profile, broker) {
  ...
  return broker.disclosureFields.map((field) => {
    ...
    if (!label || !value) throw new Error("profile_missing_required_removal_identifier");
    return `${label}: ${value}`;
  });
}
...
export async function runRemovalSubmission({
  ...
  if (!["assisted_allow_once", "finite_campaign_grant"].includes(approvalBoundary)) throw new Error("rightout_approval_boundary_invalid");

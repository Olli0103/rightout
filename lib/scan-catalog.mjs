...
export function buildCombinedScanCatalog(coreCatalog, parityCatalog) {
  ...
  for (const entry of rows(coreCatalog)) {
    ...
    } else combined.set(entry.id, entry);
  }

  for (const route of rows(parityCatalog)) {
    if (!route || typeof route !== "object" || Array.isArray(route) || typeof route.id !== "string") continue;
    ...
    if (existing?.scan?.supported === true || hasFailClosedScanGate(existing)) continue;
    combined.set(route.id, {
      id: route.id,
      name: route.name,
...
export function scanCoverage(catalog) {
  ...
  return {
    ...
    human_only_controller_portal_lanes: rows(catalog).filter((entry) => (
      entry?.category === "data_broker"
      && entry.human_only === true
      && entry.process_class === "eu_controller_portal_erasure"
    )).length,
  };
}

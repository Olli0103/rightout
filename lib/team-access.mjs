...
export function validateTeamAccess(value, configuredProfileIds) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("rightout_team_access_invalid");
  const configured = new Set(configuredProfileIds);
  const members = Object.entries(value).map(([memberId, record]) => {
    if (!SAFE_MEMBER_ID.test(memberId) || !record || typeof record !== "object" || Array.isArray(record)) throw new Error("rightout_team_access_invalid");
    if (Object.keys(record).some((key) => !["role", "sessionBindingDigest", "authorizedProfileIds"].includes(key))) throw new Error("rightout_team_access_invalid");
    if (!ROLES.has(record.role) || !SAFE_SHA256.test(record.sessionBindingDigest ?? "")) throw new Error("rightout_team_access_invalid");
    if (
      !Array.isArray(record.authorizedProfileIds) || record.authorizedProfileIds.length < 1 || record.authorizedProfileIds.length > 100
  ...
  if (members.length < 1 || members.length > 100 || !members.some((member) => member.role === "owner")) throw new Error("rightout_team_access_invalid");
  if (new Set(members.map((member) => member.session_binding_digest)).size !== members.length) throw new Error("rightout_team_access_invalid");
...
export function resolveTeamMember(value, configuredProfileIds, context) {
  ...
  if (!member) throw new Error("rightout_team_session_unauthorized");

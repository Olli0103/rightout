...
function cleanVerificationHandle(value) {
  if (typeof value !== "string" || !SAFE_VERIFY_HANDLE.test(value)) throw new Error("invalid_verification_handle");
  return value;
}

function cleanStringArray(values, pattern, max = 20) {
  if (!Array.isArray(values) || values.length < 1 || values.length > max) throw new Error("rightout_verification_attestation_required");
...
function cleanProfileDigests(value, profileIds) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("rightout_verification_attestation_required");
...
export function browserVerificationProfileDigest({ browserControlBaseUrl, browserProfile, browserBackendMode }) {
  let base;
  try { base = new URL(browserControlBaseUrl); } catch { throw new Error("rightout_browser_webmail_profile_required"); }
  if (
...
export function validateVerificationPreflight({ input, catalog, profilePayload, imapTransport, attestations }) {
  ...
  if (imapTransportDigest(imap) !== cleanAttestations.imapTransportDigest) throw new Error("rightout_verification_snapshot_changed");
...
export function validateBrowserVerificationPreflight({ input, catalog, profilePayload, browserControl, attestations }) {
  ...
  if (digest !== cleanAttestations.browserProfileDigest) throw new Error("rightout_verification_snapshot_changed");
...
export function verificationOpenApprovalDescription(input, broker) {
  ...
  if (!Array.isArray(domains) || domains.length < 1) throw new Error("unsupported_verification_lane");

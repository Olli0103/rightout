...
OpenClaw owns eager activation-time SecretRef resolution, native approval, state-directory resolution, browser sandbox, and Gateway execution. RightOut owns opaque schemas, exact-scope single-use bindings, provider-authorization and catalog policy, transport/domain restrictions, contained atomic encrypted state files, ciphertext listing tokens, and sanitized reports. Operators own subject authority, written provider permissions, accounts, approval routing, Cron scope, tool policy, and process isolation.
...
| Cross-action widening | ten independent approval bindings, including local key rotation |
| Brave result URL leaks at rest | never persisted or returned |
| Separately authorized browser candidate leaks at rest | AES-256-GCM vault under SecretRef key; opaque handle only |
...
| Direct Gateway exposure | deny every manifest tool with `replaySafe: false` on `gateway.tools.deny`; audit otherwise |

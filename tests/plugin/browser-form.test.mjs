...
test("browser form fills PII only inside the host bridge and returns an opaque proof", async () => {
  const fetchImpl = mockFetch([
    json({ ok: true, targetId: "tab-1", url: "https://suppression.peopleconnect.us/" }),
    json({
  ...
  assert.deepEqual(fetchImpl.calls[2].parsedBody, {
    kind: "fill", fields: [{ ref: "e1", type: "text", value: "avery@example.invalid" }], targetId: "tab-1",
  });
